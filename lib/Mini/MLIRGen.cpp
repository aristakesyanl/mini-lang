...
private:
  ...
  mlir::mini_lang::FuncOp mlirGen(PrototypeAST &proto) {
    ...
    return builder.create<mlir::mini_lang::FuncOp>(location, proto.getName(),
                                             funcType);
  }

  /// Emit a new function and add it to the MLIR module.
  mlir::mini_lang::FuncOp mlirGen(FunctionAST &funcAST) {
    // Create a scope in the symbol table to hold variable declarations.
    ScopedHashTableScope<llvm::StringRef, mlir::Value> varScope(symbolTable);

    // Create an MLIR function for the given prototype.
    builder.setInsertionPointToEnd(theModule.getBody());
    mlir::mini_lang::FuncOp function = mlirGen(*funcAST.getProto());
    if (!function)
      return nullptr;
  ...
  /// data in an Attribute attached to a `mini.constant` operation.
  ...
  ///   %0 = "mini.constant"() {value: dense<tensor<2x3xf64>,
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    if (callee == "transpose") {
      if (call.getArgs().size() != 1) {
        emitError(location, "MLIR codegen encountered an error: mini.transpose "
  ...
  mlir::Value mlirGen(ExprAST &expr) {
    switch (expr.getKind()) {
    case mini_lang::ExprAST::Expr_BinOp:
      return mlirGen(cast<BinaryExprAST>(expr));
    case mini_lang::ExprAST::Expr_Var:
      return mlirGen(cast<VariableExprAST>(expr));
    case mini_lang::ExprAST::Expr_Literal:
      return mlirGen(cast<LiteralExprAST>(expr));
    case mini_lang::ExprAST::Expr_Call:
      return mlirGen(cast<CallExprAST>(expr));
    case mini_lang::ExprAST::Expr_Num:
